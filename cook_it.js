    ...
    .then(IiR_results => {
      ...
      ingredients_t
        ...
        .then(ingredient_results => {
          ...
          for (var i = 0; i < ingredient_results.length; i += 1) {
            // console.log("outer for:" + (i+1));
            for (var o = 0; o < IiR_results.length; o += 1) {
              if (IiR_results[o].ingredient_id == ingredient_results[i].ingredient_id) {
                if (
                  IiR_results[i].recipe_ingredient_qty <= ingredient_results[o].ingredient_total ||
                  ingredient_results[o].ingredient_total == null
                ) {
                  ...
                  new_inv.push(
                    ingredient_results[i].ingredient_total - IiR_results[o].recipe_ingredient_qty
                  );
          ...
          if (flag_can_cook) {
            ...
            if (ingredients_id_array.length == new_inv.length) {
              for (let i = 0; i < ingredients_id_array.length; i += 1) {
                if (new_inv > 0) {
                  query_str +=
                    'UPDATE ingredients SET ingredient_total = ' +
                    new_inv[i] +
                    ' WHERE ingredient_id = ' +
                    ingredients_id_array[i] +
                    "'; ";
                } else {
                  query_str +=
                    'UPDATE ingredients SET ingredient_total = 0, ingredient_expiration_date = null WHERE ingredient_id = ' +
                    ingredients_id_array[i] +
                    '; ';
                  new_inv[i] = null;
                }
