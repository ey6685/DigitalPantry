...
$(document).on('click', '#showRecipeDetails', async function () {
  ...
  // Get recipe ID
  var recipeId = $(this).closest('.card').attr('recipe-id')
  ...
  $.each(ingredientsRequired, function (index, item) {
    ...
    if (item.ingredient_amount_required > item.ingredient_sum){
      // make entire row red
      ingredientStatus = `<tr data-id=${item.ingredient_id} style="background-color:#ff6961">`
    }
    var $tr = $(ingredientStatus).append(
      $('<td id="ingredient_name" style="color:black;">').text(item.ingredient_name),
      $('<td id="ingredient_amount_required" style="color:black;">').text(item.ingredient_amount_required+" "+item.ingredient_unit_of_measurement),
      $('<td id="ingredient_amount_available" style="color:black;">').text(item.ingredient_sum+" "+item.ingredient_unit_of_measurement))
      // if available amount of ingredient is less than required amount of ingredient
      if (item.ingredient_amount_required > item.ingredient_sum){
        $tr.append(
          $('<td style="background-color:white; border:none;">').append($('<button style=background:none; class="btn bg-success" id="add-missing-ingredient-btn" type="button">').text('Add')))
      }
      else{
    ...
    $.each(item.recipe_data, function getIngredients(index, thing) {
      if (recipeId == thing.recipe_id) {
        ...
        $.each(ingredientOnHand, function (index, single) {
          ...
          ingredientsRequired[index]["ingredient_amount_required"] = single.amount_of_ingredient_needed
        })
        found = true
        return false
      }
...
$(document).on('click', '#add-missing-ingredient-btn', function() {
  ...
  var $ingredientAmountAvailable = $(this).closest('tr').find('#ingredient_amount_available')
...
$(document).on('focusout', '#ingredient_amount_available', function(){
  ingredientId = $(this).closest('tr').attr('data-id')
  // Set ingredient table cell to not editable
  $(this).closest('tr').find('#ingredient_amount_available').attr('contenteditable','false')
  // Get new ingredient amount
  var ingredientAmountAvailableString = $(this).closest('tr').find('#ingredient_amount_available').text()
  // Get amount required
  var ingredientAmountRequiredString = $(this).closest('tr').find('#ingredient_amount_required').text()
