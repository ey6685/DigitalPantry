/*
This file is suppsoed to suggest recipe to cook based on the expired ingredient, but Patrick made a simplified version of this 
*/
    ...
    init.then(function(results){
        ...
        async.forEach(ingredientsSorted, function(item,callback) {
            ...
            test.then(function(results){
                ...
                recipePromise.then(function(results){
...
function getExpiringIngredients() {
    return new Promise((resolve, reject) => {
        var query = "SELECT ingredient_id,ingredient_total,DATE(ingredient_expiration_date) as date FROM ingredients ORDER BY ingredient_expiration_date ASC";
...
function getIdsOfIngredientsForSingleRecipe(recipeIdOfExpiringIngredient) {
    return new Promise((resolve, reject) => {
        query = "SELECT ingredient_id,recipe_ingredient_qty FROM recipe_ingredient WHERE recipe_id="+recipeIdOfExpiringIngredient;
        db.query(query, function(err, results){
            if (err){
                reject(err);
            }
